...
class ViewController: UIViewController
{
    ...
    {
        imgView.image = nil // 이미지 뷰의 이미지 없애기
    }
    
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        let touch = touches.first! as UITouch  // 발생한 터치이벤트 가져오기
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        ...
        let currPoint = touch.location(in: imgView) // 터치된 위치를 currPoint에 저장
        
        
        // 현재 이미지 뷰에 있는 이미지를 이미지뷰 크기로 그림
        imgView.image?.draw(in: CGRect(x: 0, y: 0, width: imgView.frame.size.width, height: imgView.frame.size.height))
        
        // 이전에 이동된 위치(lastPoint)로 시작위치를 이동
